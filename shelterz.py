...
def get_Lld_posts():
    return cnnOne.run("SELECT RecDte,Country,City,Suburb,NoOfRooms,Rent,ContactNo FROM AccommoRecs WHERE Country = :cntry and City = :cty and Suburb = :sbb and RS = :rs", cntry=cntry_txt, cty=cty_txt,sbb=sbb_txt,rs=RS) # vacant spaces

@st.cache_data(ttl=600)
def get_Tnt_posts():
    return cnnOne.run("SELECT RecDte,Country,City,Suburb,NoOfRooms,ContactNo FROM AccommoRecs WHERE Country = :cntryi and City = :ctyi and Suburb = :sbbi and RS = :rsi", cntryi=Country_txt, ctyi=City_txt,sbbi=Suburb_txt,rsi=RS)  # wanted spaces
