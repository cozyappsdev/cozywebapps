...
class DatabaseConnection:
    ...
    def get_instance(db_config):
        if DatabaseConnection._instance is None:
            DatabaseConnection._instance = pg8000.native.Connection(**db_config)            
...
def get_connection():
    try:
        ...
        # Specifically handles £no connection", timeouts, or network refusal
        st.error(f"Backend Connection Failed: Cannot reach server. Check your internet connection.") # {e}
        return None
    except DatabaseError as e:
        # Handles database-side errors (e.g. authentication, missing DB)
        st.error(f"Database Error: Cannot connect to database. Check to ensure the database is not missing and that your credentials are correct.") #{e}")
        return None
    
cnnOne = get_connection()
...
def get_Lld_posts():
    return cnnOne.run("SELECT * FROM AccommoRecs WHERE Country = :cntry and City = :cty and Suburb = :sbb and RS = :rs", cntry=cntry_txt, cty=cty_txt,sbb=sbb_txt,rs=RS) # vacant spaces
    
def get_Tnt_posts():
    return cnnOne.run("SELECT * FROM AccommoRecs WHERE Country = :cntryi and City = :ctyi and Suburb = :sbbi and RS = :rsi", cntryi=Country_txt, ctyi=City_txt,sbbi=Suburb_txt,rsi=RS)  # wanted spaces
   
@st.cache_data(ttl=600)
def count_males():
    result = cnnOne.run("SELECT COUNT(*) FROM AccommoRecs WHERE RS = 'ws' or RS = 'vs'")
    return result[0][0]

@st.cache_data(ttl=600)
def count_ref_no():
    result = cnnOne.run("SELECT COUNT(*) FROM AccommoRecs WHERE EntryNo = :refidno", refidno=EntryNo)
    return result[0][0]   

st.title("Accommodation Market")

st.write("Bringing the meeting place for accommodation seekers and suppliers conveniently in your own home or office!")
...
with st.expander("", expanded=is_expanded):    
    with st.form("Add_a_Post"):
        if Tsk == "Enter Details Of Accommodation You Want To Rent Out":            
            ...
            Country = st.text_input("Enter COUNTRY where accommodation is available")  
            City = st.text_input("Enter CITY name")
            Suburb = st.text_input("Enter SUBURB name")
            NoOfRooms = st.text_input("NUMBER OF ROOMS to be rented out")
            Rent = st.text_input("Total MONTHLY RENT amount in your currency e.g. ZWD250.00/$250.00/£290.00")
            EntryNo = st.text_input("Create and enter a UNIQUE IDENTIFIER for the post.")
            ContactNo = st.text_input("Enter your BUSINESS CONTACT NUMBER")             
            RS=""          
            ...
            City = st.text_input("Enter CITY name")
            Suburb = st.text_input("Enter SUBURB name")
            NoOfRooms = st.text_input("NUMBER OF ROOMS wanted")           
            EntryNo = st.text_input("Create and enter a UNIQUE IDENTIFIER for the post.")
            ContactNo = st.text_input("Enter your BUSINESS CONTACT NUMBER") 
            RS=""           
            if st.form_submit_button("Submit"):               
...
if Tsk == "View Posts Of Accommodation Available In Areas Where You Are Looking For Accommodation":
    ...
    if st.button("View Available Accommodation"):
        query = "SELECT * FROM AccommoRecs WHERE Country = :cntryi and City = :ctyi and Suburb = :sbbi and RS = :rsi"
        llds_posts = cnnOne.run(query, cntryi=cntry_txt, ctyi=cty_txt,sbbi=sbb_txt,rsi=RS)  # wanted spaces
        ...
        if llds_posts:         
            ...
            for llds_post in llds_posts:                
                llds_data.append({
                    'Date_Posted': llds_post[1],
                    'Suburb': llds_post[4],
                    'Rooms_Available': llds_post[5],
                    'Montly_Rent_Amt': llds_post[6],              
                    'Bus_Contact_No': llds_post[8],
                    # 'Categ': llds_post[9]
                    })
            st.write("")
    ...
    Country_txt = st.text_input("Enter COUNTRY name")
    City_txt = st.text_input("Enter CITY name")
    Suburb_txt = st.text_input("Enter SUBURB name")                   
    #RS=""                      
    RS = "ws"  # wanted spaces
    if st.button("View Searches for Accommodation"):

        query = "SELECT * FROM AccommoRecs WHERE Country = :cntryx and City = :ctyx and Suburb = :sbbx and RS = :rsx"
        tnts_posts = cnnOne.run(query, cntryx=Country_txt, ctyx=City_txt,sbbx=Suburb_txt,rsx=RS)  # wanted spaces        
        ...
        if tnts_posts:         
            ...
            for tnts_post in tnts_posts: 
                tnts_data.append({
                    'Date_Posted': tnts_post[1],
                    'Suburd_Wanted': tnts_post[4],
                    'No_of_Rooms_Wanted': tnts_post[5],                
                    'Bus_Contact_No': tnts_post[8],
                    # 'Categ': tnts_post[9]
                    })
            st.write("")
