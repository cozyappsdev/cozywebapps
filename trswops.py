...
positions=["","SCHOOL HEAD","SCHOOL DEPUTY HEAD"] 
...
page = st.sidebar.radio(
    "Go to:",
    ["Welcome Page", "Class Teacher Swaps", "School Heads Swaps"]    
)
...
def show_class_teacher_swops():
    st.title("Class Teacher Swaps")
    st.write("Handles entry, use, and management of class teacher swop data")
    
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # with st.form("teacher_swop_form"):

    st.subheader("Class Teacher Swaps")
    ...
    # Place 'View ALL Posts' logic here 
        ...
        if st.button("View ALL Posts"):
            ...
            #query = "SELECT * FROM TrPosts"  # WHERE RS = :rs or RS = :rsii"
            #all_posts = cnnOne.run(query, rs="vs",rsii="ws")
            all_posts = cnnOne.run("SELECT * FROM TrPosts WHERE RecID>0") 
            ...
            if all_posts:
                ...
                for one_post in all_posts:                
                    all_data.append({
                        'Date_Posted': one_post[1],
                        'Username': one_post[2],                                             
                        'Bus_Cont_No': one_post[3],
                        'School_level': one_post[4],
                ...
                pd.DataFrame()    
                # Create a DataFrame from the list
                allposts_df = pd.DataFrame(all_data)        
            
                # Display the DataFrame in Streamlit
                st.dataframe(allposts_df)
    ...
    if st.sidebar:
        st.sidebar.subheader("Data entry instructions:")    
        ...
        st.sidebar.write("   Keep all your post's UNIQUE IDENTIFIERS in a safe place: You will need them when it's time to delete your posts.")

def show_school_heads_swops():
    st.title("School Heads Swaps")
    st.write("Handles entry, use, and management of school heads swop data")
    
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # with st.form("school_head_form"):

    st.subheader("School Heads Swaps")

    st.write("Where all school heads intending to swop can EASILY find each other and conclude their swapping deals!")
    ...
    if cnnOne:
        ...
        cnnOne.run("""
                ...
                CREATE TABLE IF NOT EXISTS HMSwops (
    ...
    with st.expander("", expanded=is_expanded):    
        with st.form("Add_a_Post"):
            if Tsk == "ENTER MY DETAILS":
                RecDate = st.text_input("Entry Date", value=formatted_curdate) 
                UName = st.text_input("Enter your USERNAME")           
                BusContactNo = st.text_input("Enter your BUSINESS CONTACT NUMBER")
                SchoolLevel = st.selectbox("Select SCHOOL LEVEL", options=schoollevel)
                PositionHeld = st.selectbox("Select POSITION HELD", options=positions)                
                CurrentDistrict = st.selectbox("Select CURRENT DISTRICT where you are working", options=districts)
                DesiredDistrict = st.selectbox("Select TARGET DISTRICT", options=districts)
                EUId = st.text_input("Create and Enter UNIQUE IDENTIFIER for this subject's post. (Keep this; you will be required to use it later).")

                # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                @st.cache_data(ttl=600)
                def count_EUId():
                    result = cnnOne.run("SELECT COUNT(*) FROM HMSwops WHERE EnryUId = :euid", euid=EUId)
                    return result[0][0]
                # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                ...
                if st.form_submit_button("Submit"):
                    ...
                    if refid_count > 0:
                        ...
                        if RecDate =="" or UName =="" or BusContactNo == "" or SchoolLevel == "" or PositionHeld == "" or CurrentDistrict == "" or DesiredDistrict == "" or EUId == "":
                            st.error("Enter data in all the data fields.")
                        else:                       
                            if cnnOne:                        
                                cnnOne.run("INSERT INTO HMSwops (RecDte, Unme, BusContNo, SchLevel, PositionHeld, CurDist, DesiredDist, EnryUId) VALUES (:recdte, :unme, :buscontno, :schlevel, :positionheld, :curdist, :desireddist, :enryuid)",
                                    recdte=RecDate, unme=UName, buscontno=BusContactNo, schlevel=SchoolLevel, positionheld=PositionHeld, curdist=CurrentDistrict, desireddist=DesiredDistrict, enryuid=EUId)                                       
                                #cnnOne.commit()
                                st.cache_data.clear() # Reset cache to show updated data                        
                                st.toast("Record successfully inserted.")                               
    ...
    if Tsk == "FIND MY SWOP MATCHES":
         ...
         # member's details
        membersSchLvl = st.selectbox("Enter SCHOOL LEVEL",  options=schoollevel)
        membersPH = st.selectbox("Select POSITION HELD", options=positions)       
        membersCurDist = st.selectbox("Select CURRENT DISTRICT where you are working", options=districts)
        membersDesiredDist = st.selectbox("Select DISTRICT where you wish to go", options=districts) 
       
        if st.button("Find My Swop Matches"):
            query = "SELECT * FROM HMSwops WHERE SchLevel = :ksl and PositionHeld = :kph and CurDist = :kcd and DesiredDist= :kdd"                   
            matching_posts = cnnOne.run(query, ksl = membersSchLvl, kph = membersPH, kcd=membersDesiredDist, kdd=membersCurDist)          
            ...
            if matching_posts:         
                ...
                for matching_post in matching_posts:            
                    matching_data.append({
                        ...
                        'Bus_Cont_No': matching_post[3],
                        'School_level': matching_post[4],
                        'Position_Held': matching_post[5],                        
                        'Current_District': matching_post[6],                               
                        'District_Desired': matching_post[7]                    
                        })
                st.write("")
                st.write(f"LIST OF MEMBERS WITH WHOM YOU MAY SWOP")
    ...
    # Place 'View ALL Posts' logic here 
        ...
        if st.button("View ALL Posts"):
            ...
            #query = "SELECT * FROM TrPosts"  # WHERE RS = :rs or RS = :rsii"
            #all_posts = cnnOne.run(query, rs="vs",rsii="ws")
            all_posts = cnnOne.run("SELECT * FROM HMSwops WHERE RecID>0") 
            ...
            if all_posts:
                ...
                for one_post in all_posts:                
                    all_data.append({
                        ...
                        'Bus_Cont_No': one_post[3],
                        'School_level': one_post[4],
                        'Position_Held': one_post[5],                        
                        'Current_District': one_post[6],                               
                        'District_Desired': one_post[7],
                        # 'Post_RefId': one_post[9]                    
                        })
                st.write("")
                st.write(f"LIST OF ALL THE POSTS")
            ...
            if st.button("Delete Entry"):                
                if EUId =="":
                    ...
                    if refid_count < 1:
                        ...
                        if cnnOne:                        
                            cnnOne.run("DELETE FROM HMSwops WHERE EnryUId = :pri", pri=EUId)
                            # cnnOne.commit()                                               
                            st.cache_data.clear() # Reset cache to show updated data
                            st.toast(f"Post with Unique Identifier {EUId} successfully deleted!")
...
if page == "Welcome Page":
    show_welcome()
elif page == "Class Teacher Swaps":
    show_class_teacher_swops()    
elif page == "School Heads Swaps":
    show_school_heads_swops()
